...
public class WaspP extends Entity {
    ...
    public WaspP(Main g, World world, float x, float y, float lifetime, float size) {
        ...
        AdvBodyPart playerPart = new AdvBodyPart(
                g.atlas.createSprite("bee_enemy", 1),
                -size * Main.METER,
                -size * Main.METER,
                2f * size * Main.METER,
                2f * size * Main.METER,
    ...
    public void updateLife(float delta) {
        ...
        if (distance.len() <= 600f) {
            getFsa().changeState(getFsa().getState("look"));
